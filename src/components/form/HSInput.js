const HSInput = ({
  ...
  return (
    <div className="flex flex-col gap-2 w-full group">
      ...
      {type === "input" ? (
        <div
          className={`${style} relative text-primary  duration-100 outline-none justify-between flex items-center gap-6 px-2  w-full rounded-md font-medium border-2 group-hover:border-primary`}
        >
          {icon && <p>{icon}</p>}
